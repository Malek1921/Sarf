...
import usePurchases from "../../store/purchases/usePurchases"; // Import purchases store

function AddPurchase({ setActiveTab }) {
  const { products } = useProducts();
  const { addPurchase } = usePurchases(); // Get addPurchase function
  ...
  const submit = (data) => {
    const newPurchase = {
      ...
      id: Math.floor(Math.random() * 9000) + 1000, // Better ID generation
    };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <form onSubmit={handleSubmit(submit)} className="space-y-10">
          ...
          <div className="flex gap-6">
            <button
              type="submit"
              className="flex-1 bg-slate-900 text-white py-5 rounded-xl font-bold text-xl hover:bg-slate-800"
            >
              Add Purchase
            </button>
            <button
